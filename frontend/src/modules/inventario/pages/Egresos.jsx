...
import { GET_NOTAS_EGRESO, GET_PRODUCTOS, GET_ALMACENES } from '../graphql/queries'
import { CREAR_NOTA_EGRESO, AGREGAR_DETALLE_EGRESO } from '../graphql/mutations'

export default function Egresos() {
  const { data, loading, refetch } = useQuery(GET_NOTAS_EGRESO)
  const { data: dataProductos }    = useQuery(GET_PRODUCTOS)
  const { data: dataAlmacenes }    = useQuery(GET_ALMACENES)
  const [crearNota]                = useMutation(CREAR_NOTA_EGRESO)
  const [agregarDetalle]           = useMutation(AGREGAR_DETALLE_EGRESO)
  ...
  const handleCrearNota = async (e) => {
    ...
    try {
      ...
      if (res.crearNotaEgreso.ok) {
        setMensaje('Nota de egreso creada')
        setNotaSel(res.crearNotaEgreso.notaEgreso)
        setModo('detalle')
        refetch()
      }
    } catch (err) {
  ...
  const handleAgregarDetalle = async (e) => {
    ...
    try {
      const { data: res } = await agregarDetalle({
        variables: {
          idEgreso:    parseInt(notaSel.idEgreso),
      ...
      if (res.agregarDetalleEgreso.ok) {
        setMensaje(res.agregarDetalleEgreso.mensaje)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-800">Notas de Egreso</h2>
        <button
      ...
      {modo === 'nuevo' && (
        <form onSubmit={handleCrearNota} className="bg-white p-4 rounded-lg shadow mb-6">
          <h3 className="font-semibold text-gray-700 mb-3">Nueva Nota de Egreso</h3>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {data?.notasEgreso.map((n) => (
                <tr key={n.idEgreso} className="hover:bg-gray-50">
                  <td className="px-4 py-3">{n.idEgreso}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {n.detalles.map((d) => (
                        <span key={d.idDetalleEgreso} className="bg-red-100 text-red-700 text-xs px-2 py-0.5 rounded-full">
